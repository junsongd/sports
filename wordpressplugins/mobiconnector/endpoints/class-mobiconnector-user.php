...
class MobiConnectorUser extends  WP_REST_Controller{
	...
	public function forgot_password( $request ) {
		...
		**/
		$parameters = $request->get_params();
		// kiểm tra username hay email đã tồn tại chưa
		 $user_id = email_exists( $parameters['email'] );  
		//var_dump($user_id);
		if($user_id == false) {
	...
	public function update_profile($request) {
		...
		***/
		...
		global $blog_id, $wpdb;
		$attachment = wp_get_attachment_url( (int) $user_meta[$this->getAvatarMetaKey()]);
	...
	*/
	public function update_user_avatar($user_id, $image_base64string) {
		...
		if(!empty($image_base64string)) {
			...
			update_user_meta($user_id, $this->getAvatarMetaKey(), $attach_id);
	...
	public function getAvatarMetaKey(){
		global $blog_id, $wpdb;
